...
$sms_log_add->Page_Render();
?>
<?php include_once "header.php"; ?>
<script>
var fsms_logadd, currentPageID;
loadjs.ready("head", function() {

	// Form object
	currentPageID = ew.PAGE_ID = "add";
	fsms_logadd = currentForm = new ew.Form("fsms_logadd", "add");

	// Validate form
	fsms_logadd.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($sms_log_add->sms_log_branch_id->Required) { ?>
			...
			<?php if ($sms_log_add->sms_log_message->Required) { ?>
				...
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $sms_log_add->sms_log_message->caption(), $sms_log_add->sms_log_message->RequiredErrorMessage)) ?>");
			<?php } ?>
			<?php if ($sms_log_add->sms_log_to->Required) { ?>
				elm = this.getElements("x" + infix + "_sms_log_to");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $sms_log_add->sms_log_to->caption(), $sms_log_add->sms_log_to->RequiredErrorMessage)) ?>");
			<?php } ?>
			<?php if ($sms_log_add->sms_log_date->Required) { ?>
				elm = this.getElements("x" + infix + "_sms_log_date");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $sms_log_add->sms_log_date->caption(), $sms_log_add->sms_log_date->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_sms_log_date");
				if (elm && !ew.checkDateDef(elm.value))
					return this.onError(elm, "<?php echo JsEncode($sms_log_add->sms_log_date->errorMessage()) ?>");
	...
	// Form_CustomValidate
	fsms_logadd.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
...
<?php $sms_log_add->showPageHeader(); ?>
<?php
$sms_log_add->showMessage();
?>
<form name="fsms_logadd" id="fsms_logadd" class="<?php echo $sms_log_add->FormClassName ?>" action="<?php echo CurrentPageName() ?>" method="post">
<?php if ($Page->CheckToken) { ?>
<input type="hidden" name="<?php echo Config("TOKEN_NAME") ?>" value="<?php echo $Page->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="sms_log">
<input type="hidden" name="action" id="action" value="insert">
<input type="hidden" name="modal" value="<?php echo (int)$sms_log_add->IsModal ?>">
<div class="ew-add-div"><!-- page* -->
<?php if ($sms_log_add->sms_log_branch_id->Visible) { // sms_log_branch_id ?>
...
<?php } ?>
<?php if ($sms_log_add->sms_log_sms_api_id->Visible) { // sms_log_sms_api_id ?>
...
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_sms_log_sms_api_id"><?php echo EmptyValue(strval($sms_log_add->sms_log_sms_api_id->ViewValue)) ? $Language->phrase("PleaseSelect") : $sms_log_add->sms_log_sms_api_id->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($sms_log_add->sms_log_sms_api_id->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($sms_log_add->sms_log_sms_api_id->ReadOnly || $sms_log_add->sms_log_sms_api_id->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_sms_log_sms_api_id',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $sms_log_add->sms_log_sms_api_id->Lookup->getParamTag($sms_log_add, "p_x_sms_log_sms_api_id") ?>
<input type="hidden" data-table="sms_log" data-field="x_sms_log_sms_api_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $sms_log_add->sms_log_sms_api_id->displayValueSeparatorAttribute() ?>" name="x_sms_log_sms_api_id" id="x_sms_log_sms_api_id" value="<?php echo $sms_log_add->sms_log_sms_api_id->CurrentValue ?>"<?php echo $sms_log_add->sms_log_sms_api_id->editAttributes() ?>>
</span>
...
<?php } ?>
<?php if ($sms_log_add->sms_log_to->Visible) { // sms_log_to ?>
	<div id="r_sms_log_to" class="form-group row">
		<label id="elh_sms_log_sms_log_to" for="x_sms_log_to" class="<?php echo $sms_log_add->LeftColumnClass ?>"><?php echo $sms_log_add->sms_log_to->caption() ?><?php echo $sms_log_add->sms_log_to->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $sms_log_add->RightColumnClass ?>"><div <?php echo $sms_log_add->sms_log_to->cellAttributes() ?>>
<span id="el_sms_log_sms_log_to">
<textarea data-table="sms_log" data-field="x_sms_log_to" name="x_sms_log_to" id="x_sms_log_to" cols="35" rows="4" placeholder="<?php echo HtmlEncode($sms_log_add->sms_log_to->getPlaceHolder()) ?>"<?php echo $sms_log_add->sms_log_to->editAttributes() ?>><?php echo $sms_log_add->sms_log_to->EditValue ?></textarea>
</span>
<?php echo $sms_log_add->sms_log_to->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($sms_log_add->sms_log_date->Visible) { // sms_log_date ?>
	<div id="r_sms_log_date" class="form-group row">
		<label id="elh_sms_log_sms_log_date" for="x_sms_log_date" class="<?php echo $sms_log_add->LeftColumnClass ?>"><?php echo $sms_log_add->sms_log_date->caption() ?><?php echo $sms_log_add->sms_log_date->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $sms_log_add->RightColumnClass ?>"><div <?php echo $sms_log_add->sms_log_date->cellAttributes() ?>>
<span id="el_sms_log_sms_log_date">
<input type="text" data-table="sms_log" data-field="x_sms_log_date" data-format="2" name="x_sms_log_date" id="x_sms_log_date" maxlength="10" placeholder="<?php echo HtmlEncode($sms_log_add->sms_log_date->getPlaceHolder()) ?>" value="<?php echo $sms_log_add->sms_log_date->EditValue ?>"<?php echo $sms_log_add->sms_log_date->editAttributes() ?>>
<?php if (!$sms_log_add->sms_log_date->ReadOnly && !$sms_log_add->sms_log_date->Disabled && !isset($sms_log_add->sms_log_date->EditAttrs["readonly"]) && !isset($sms_log_add->sms_log_date->EditAttrs["disabled"])) { ?>
<script>
loadjs.ready(["fsms_logadd", "datetimepicker"], function() {
	ew.createDateTimePicker("fsms_logadd", "x_sms_log_date", {"ignoreReadonly":true,"useCurrent":false,"format":2});
});
</script>
<?php } ?>
</span>
<?php echo $sms_log_add->sms_log_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$sms_log_add->IsModal) { ?>
<div class="form-group row"><!-- buttons .form-group -->
	<div class="<?php echo $sms_log_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit"><?php echo $Language->phrase("AddBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-href="<?php echo $sms_log_add->getReturnUrl() ?>"><?php echo $Language->phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<?php
$sms_log_add->showPageFooter();
if (Config("DEBUG"))
