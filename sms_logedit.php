		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
				...
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($sms_log_edit->sms_log_branch_id->errorMessage()) ?>");
			...
			<?php } ?>
				elm = this.getElements("x" + infix + "_sms_log_sms_api_id");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($sms_log_edit->sms_log_sms_api_id->errorMessage()) ?>");
...
<input type="text" data-table="sms_log" data-field="x_sms_log_branch_id" name="x_sms_log_branch_id" id="x_sms_log_branch_id" size="30" maxlength="12" placeholder="<?php echo HtmlEncode($sms_log_edit->sms_log_branch_id->getPlaceHolder()) ?>" value="<?php echo $sms_log_edit->sms_log_branch_id->EditValue ?>"<?php echo $sms_log_edit->sms_log_branch_id->editAttributes() ?>>
</span>
...
<input type="text" data-table="sms_log" data-field="x_sms_log_sms_api_id" name="x_sms_log_sms_api_id" id="x_sms_log_sms_api_id" size="30" maxlength="12" placeholder="<?php echo HtmlEncode($sms_log_edit->sms_log_sms_api_id->getPlaceHolder()) ?>" value="<?php echo $sms_log_edit->sms_log_sms_api_id->EditValue ?>"<?php echo $sms_log_edit->sms_log_sms_api_id->editAttributes() ?>>
</span>
...
<?php if ($sms_log_edit->sms_log_message->Visible) { // sms_log_message ?>
	<div id="r_sms_log_message" class="form-group row">
		<label id="elh_sms_log_sms_log_message" for="x_sms_log_message" class="<?php echo $sms_log_edit->LeftColumnClass ?>"><?php echo $sms_log_edit->sms_log_message->caption() ?><?php echo $sms_log_edit->sms_log_message->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
