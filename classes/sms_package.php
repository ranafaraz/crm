...
class sms_package extends DbTable
{
	...
	public function __construct()
	{
		...
		// sms_pkg_branch_id
		$this->sms_pkg_branch_id = new DbField('sms_package', 'sms_package', 'x_sms_pkg_branch_id', 'sms_pkg_branch_id', '`sms_pkg_branch_id`', '`sms_pkg_branch_id`', 3, 12, -1, FALSE, '`EV__sms_pkg_branch_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
	...
	public function getSqlSelectList() // Select for List page
	{
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `sms_api_mask` FROM `sms_api` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`sms_api_id` = `sms_package`.`sms_pkg_sms_api_id` LIMIT 1) AS `EV__sms_pkg_sms_api_id`, (SELECT `branch_name` FROM `branch` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`branch_id` = `sms_package`.`sms_pkg_branch_id` LIMIT 1) AS `EV__sms_pkg_branch_id` FROM `sms_package`" .
			") `TMP_TABLE`";
	...
	protected function useVirtualFields()
	{
		...
		if (ContainsString($orderBy, " " . $this->sms_pkg_sms_api_id->VirtualExpression . " "))
			return TRUE;
		if ($this->sms_pkg_branch_id->AdvancedSearch->SearchValue != "" ||
			$this->sms_pkg_branch_id->AdvancedSearch->SearchValue2 != "" ||
			ContainsString($where, " " . $this->sms_pkg_branch_id->VirtualExpression . " "))
			return TRUE;
		if (ContainsString($orderBy, " " . $this->sms_pkg_branch_id->VirtualExpression . " "))
			return TRUE;
		return FALSE;
	}
