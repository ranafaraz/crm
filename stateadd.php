...
loadjs.ready("head", function() {
	...
	fstateadd.lists["x_state_country_id"] = <?php echo $state_add->state_country_id->Lookup->toClientList($state_add) ?>;
	fstateadd.lists["x_state_country_id"].options = <?php echo JsonEncode($state_add->state_country_id->lookupOptions()) ?>;
...
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_state_country_id"><?php echo EmptyValue(strval($state_add->state_country_id->ViewValue)) ? $Language->phrase("PleaseSelect") : $state_add->state_country_id->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($state_add->state_country_id->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($state_add->state_country_id->ReadOnly || $state_add->state_country_id->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_state_country_id',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
		<button type="button" class="btn btn-default ew-add-opt-btn" id="aol_x_state_country_id" title="<?php echo HtmlTitle($Language->phrase("AddLink")) . "&nbsp;" . $state_add->state_country_id->caption() ?>" data-title="<?php echo $state_add->state_country_id->caption() ?>" onclick="ew.addOptionDialogShow({lnk:this,el:'x_state_country_id',url:'countryaddopt.php'});"><i class="fas fa-plus ew-icon"></i></button>
	</div>
</div>
<?php echo $state_add->state_country_id->Lookup->getParamTag($state_add, "p_x_state_country_id") ?>
<input type="hidden" data-table="state" data-field="x_state_country_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $state_add->state_country_id->displayValueSeparatorAttribute() ?>" name="x_state_country_id" id="x_state_country_id" value="<?php echo $state_add->state_country_id->CurrentValue ?>"<?php echo $state_add->state_country_id->editAttributes() ?>>
</span>
