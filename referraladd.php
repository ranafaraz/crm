...
<?php if ($referral_add->referral_desc->Visible) { // referral_desc ?>
	<div id="r_referral_desc" class="form-group row">
		<label id="elh_referral_referral_desc" class="<?php echo $referral_add->LeftColumnClass ?>"><?php echo $referral_add->referral_desc->caption() ?><?php echo $referral_add->referral_desc->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php $referral_add->referral_desc->EditAttrs->appendClass("editor"); ?>
<textarea data-table="referral" data-field="x_referral_desc" name="x_referral_desc" id="x_referral_desc" cols="35" rows="4" placeholder="<?php echo HtmlEncode($referral_add->referral_desc->getPlaceHolder()) ?>"<?php echo $referral_add->referral_desc->editAttributes() ?>><?php echo $referral_add->referral_desc->EditValue ?></textarea>
<script>
loadjs.ready(["freferraladd", "editor"], function() {
	ew.createEditor("freferraladd", "x_referral_desc", 0, 0, <?php echo $referral_add->referral_desc->ReadOnly || FALSE ? "true" : "false" ?>);
});
</script>
</span>
...
<script>
loadjs.ready(["freferraladd", "editor"], function() {
	ew.createEditor("freferraladd", "x_referral_deal_signed", 0, 0, <?php echo $referral_add->referral_deal_signed->ReadOnly || FALSE ? "true" : "false" ?>);
});
</script>
</span>
